...
pub fn add_a_r8(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.half_carry = (data1 & 0x0F) + (data2 & 0x0F) > 0x0F;
    cpu.registers.f.carry = data1 as u16 + data2 as u16 > 0xFF;
...
pub fn add_a_imm8(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.zero = result == 0;
    cpu.registers.f.subtraction = false;
    cpu.registers.f.half_carry = (data1 as u8 & 0x0F) + (data2 as u8 & 0x0F) > 0x0F;
    cpu.registers.f.carry = data1 as u16 + data2 as u16 > 0xFF;
    8
}
...
pub fn add_hl_r16(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.subtraction = false;
    cpu.registers.f.half_carry = (data1 & 0x07FF) + (data2 & 0x07FF) > 0x07FF;
    cpu.registers.f.carry = data1 as u32 + data2 as u32 > 0xFFFF;
    8
}
...
pub fn adc_a_r8(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.zero = result == 0;
    cpu.registers.f.subtraction = false;
    cpu.registers.f.half_carry = (data1 & 0x0F) + (data2 & 0x0F) + carry > 0x0F;
    cpu.registers.f.carry = data1 as u16 + data2 as u16 + carry as u16 > 0xFF;
    if register == R8::HLMem {
        8
    } else {
        4
...
pub fn sub_a_r8(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.half_carry = (data1 & 0x0F) < (data2 & 0x0F);
    cpu.registers.f.carry = (data1 as u16) < (data2 as u16);
    if register == R8::HLMem {
        8
    } else {
        4
...
pub fn sbc_a_r8(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.zero = result == 0;
    cpu.registers.f.subtraction = true;
    cpu.registers.f.half_carry = (data1 & 0x0F) < (data2 & 0x0F) + carry;
    cpu.registers.f.carry = (data1 as u16) < (data2 as u16) + carry as u16;
    if register == R8::HLMem {
        8
    } else {
        4
...
pub fn inc_r8(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.zero = result == 0;
    cpu.registers.f.subtraction = false;
    cpu.registers.f.half_carry = (data & 0x0F) + 1 > 0x0F;
    if register == R8::HLMem {
        12
    } else {
        4
...
pub fn dec_r8(cpu: &mut Cpu) -> u8 {
    ...
    cpu.registers.f.zero = result == 0;
    cpu.registers.f.subtraction = true;
    cpu.registers.f.half_carry = (data & 0x0F) == 0;
    if register == R8::HLMem {
        12
    } else {
        4
