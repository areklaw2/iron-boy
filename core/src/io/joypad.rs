...
mod test {
    ...
    #[test]
    fn buttons() {
        let mut joypad = super::Joypad::new();
        let row0: [JoypadButton; 4] = [JoypadButton::A, JoypadButton::B, JoypadButton::Select, JoypadButton::Start];

        for i in 0..row0.len() {
            joypad.button_down(row0[i]);

            joypad.mem_write(0xFF00, 0x00);
            assert_eq!(joypad.mem_read(0xFF00), 0xCF & !(1 << i));

            joypad.mem_write(0xFF00, 0x10);
            assert_eq!(joypad.mem_read(0xFF00), 0xDF & !(1 << i));

            joypad.mem_write(0xFF00, 0x20);
            assert_eq!(joypad.mem_read(0xFF00), 0xEF);

            joypad.mem_write(0xFF00, 0x30);
            assert_eq!(joypad.mem_read(0xFF00), 0xFF);

            joypad.button_up(row0[i]);
        }
    }
    ...
    fn direction() {
        let mut joypad = super::Joypad::new();
        let row2: [JoypadButton; 4] = [JoypadButton::Right, JoypadButton::Left, JoypadButton::Up, JoypadButton::Down];

        for i in 0..row2.len() {
            joypad.button_down(row2[i]);

            joypad.mem_write(0xFF00, 0x00);
            assert_eq!(joypad.mem_read(0xFF00), 0xCF & !(1 << i));

            joypad.mem_write(0xFF00, 0x10);
            assert_eq!(joypad.mem_read(0xFF00), 0xDF);

            joypad.mem_write(0xFF00, 0x20);
            assert_eq!(joypad.mem_read(0xFF00), 0xEF & !(1 << i));

            joypad.mem_write(0xFF00, 0x30);
            assert_eq!(joypad.mem_read(0xFF00), 0xFF);

            joypad.button_up(row2[i]);
        }
    }
}
