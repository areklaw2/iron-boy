...
impl Cpu {
    ...
    fn execute(&mut self) -> u8 {
        ...
        match value {
            ...
            0x2F => self.alu_instructions.cpl(opcode, &mut self.registers),
            ...
            0x37 => self.alu_instructions.scf(opcode, &mut self.registers),
            ...
            0x3F => self.alu_instructions.ccf(opcode, &mut self.registers),
    ...
    fn execute_cb(&mut self) -> u8 {
        ...
        match opcode.value {
            ...
            0x08 => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x09 => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x0A => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x0B => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x0C => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x0D => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x0E => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x0F => self.rsb_instructions.rrc(opcode, &mut self.registers, &mut self.bus),
            0x10 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x11 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x12 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x13 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x14 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x15 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x16 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x17 => self.rsb_instructions.rl(opcode, &mut self.registers, &mut self.bus),
            0x18 => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            0x19 => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            0x1A => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            0x1B => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            0x1C => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            0x1D => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            0x1E => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            0x1F => self.rsb_instructions.rr(opcode, &mut self.registers, &mut self.bus),
            code => panic!("Code {:#04X} not implemented", code),
        }
    }
}
